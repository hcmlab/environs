...
{
    if ( !env )
        env = environs::Loader::CreateInstance ( "SensorData", "Environs" );
...
{
    CVerbArg ( "OnStatus: [%s]", environs::resolveName(status) );
 ...
 */
void Observer::OnNotify ( ObserverNotifyContext * context )
{
    CVerbArg ( "onNotify: [%8X] [%10i] | %s", context->destID, context->sourceIdent, environs::resolveName(context->notification) );
 ...
 */
void Observer::OnPortalRequestOrProvided ( sp ( PortalInstance ) portal )
{
    CLog ( "OnPortal" );
}
