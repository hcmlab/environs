...
public:
	...
	*/
	void OnDeviceChanged ( const sp ( DeviceInstance ) &device, DeviceInfoFlag_t changedFlags );
	...
	*/
	void OnMessage ( const sp ( MessageInstance ) &msg, MessageInfoFlag_t changedFlags );
	...
	* @param changedFlags	Flags that indicate the object change.
	*/
	void OnData ( const sp ( FileInstance ) &fileData, FileInfoFlag_t changedFlags );
