...
public class Observer implements EnvironsObserver
{
	...
	public void OnStatus ( @Status.Value int status )
	{
		if (ChatActivity.menu != null) {
			...
			if ( status >= Status.Started ) {
				...
				//ChatActivity.StartUpdaterThread ();
				ChatActivity.enabled = true;
			}
			/*else {
				if (ChatActivity.instance != null)
					ChatActivity.instance.updaterEnabled = false;
			}*/


			// Get menu item
			final MenuItem item = ChatActivity.menu.findItem( environs.ChatApp.R.id.actionEnvStart);
			if (item != null) {
	...
	void UpdateDeviceID ()
	{
		...
		if (ChatActivity.environs != null)
			deviceID = ChatActivity.environs.GetDeviceID ();

		final int devID = deviceID;
		ChatActivity.instance.runOnUiThread(new Runnable() {
			@Override
			public void run() {
				ChatActivity.instance.setTitle ( "ChatApp 0x" + String.format ( "%X", devID ) );
			}
		});
	}
